...
class WebService: Codable {
    func downloadData<T: Codable>(fromURL: String) async -> T? {
        do {
            ...
            let (data, response) = try await URLSession.shared.data(from: url) // make the url get and wait for the response
            guard let serverResponse = response as? HTTPURLResponse else { print("Did not get a valid response"); return nil}
            guard serverResponse.statusCode == 200 else { print("Did not get HTTP 200 reponse"); return nil}
